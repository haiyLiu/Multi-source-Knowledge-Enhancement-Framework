...
class LinkGraph:
    ...
    def __init__(self, train_path: str, fact_path: str):
        ...
        for ex in examples:
            ...
            if head_id not in self.graph:
                self.graph[head_id] = {}
            if relation not in self.graph[head_id]:
                self.graph[head_id][relation] = set()
            self.graph[head_id][relation].add(tail_id)
            if tail_id not in self.back_graph:
                self.back_graph[tail_id] = {}
            if relation not in self.back_graph[tail_id]:
                self.back_graph[tail_id][relation] = set()    #对于member meronym不能直接加inv_，否则会变为inv_member meronym会造成语义误解
            self.back_graph[tail_id][relation].add(head_id)

        if os.path.exists(fact_path):
            fact_examples = json.load(open(fact_path, 'r', encoding='utf-8'))
            for ex in fact_examples:
                head_id, tail_id = ex['head_id'], ex['tail_id']
                relation = ex['relation']
                if head_id not in self.graph:
                    self.graph[head_id] = {}
                ...
                #     self.graph[tail_id] = {}
                # if 'inverse '+relation not in self.graph[tail_id]:
                #     self.graph[tail_id]['inverse '+relation] = set()    #对于member meronym不能直接加inv_，否则会变为inv_member meronym会造成语义误解
                # self.graph[tail_id]['inverse '+relation].add(head_id)
    ...
    def get_special_relation_neighbor_ids(self, entity_id: str, relation: str, inverse: bool) -> List[str]:
        if inverse:
            return list(self.back_graph[entity_id][relation])
        return list(self.graph[entity_id][relation])
