...
from wiki.src import dist_utils
...
def load_hf(object_class, model_name):
    try:
        obj = object_class.from_pretrained(model_name, local_files_only=True, device='cuda:0')
    except:
        obj = object_class.from_pretrained(model_name, local_files_only=False, device='cuda:0')
    return obj
