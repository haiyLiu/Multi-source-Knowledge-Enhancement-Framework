# 让LLM对query给出可能的结果（因为这些结果都是平等的，例如参加哪一届会议），如果truth entity在给出的结果里，就把truth entity放在top1，其余的实体按照之前候选列表中的顺序排序
import os
import re
import json
...
def save_data(item, forward_prompt, backward_prompt, forward_sorted_candidates, backward_sorted_candidates, forward_source_rank, backward_source_rank, forward_llm_rank, backward_llm_rank, forward_extracted_text, backward_extracted_text):
    answer = dict()
    ...
    answer["forward_select_candidates"] = forward_extracted_text
    answer["forward_sorted_candidates"] = forward_sorted_candidates
    

    answer["backward_prompt"] = backward_prompt
    answer["backward_select_candidates"] = backward_extracted_text
...
def construct_select_prompt(neighbors, neighbors_info, wikibase_info, entity, relation, query, candidate_entities, inverse=False):
    task_desc = "You are a good assistant to perform entity prediction. Given a goal question and a list of candidate answers to this question. You must select  as many valid and correct answers as possible from the list of candidate answers."

    neighbors_info = "; ".join(f"({i+1}) {item}" for i, item in enumerate(neighbors_info))
    ...
    wiki_text = "; ".join(f"({i+1}) {item}" for i, item in enumerate(wiki_text))
    ...
    candidate_entities = "; ".join(f"({i+1}) {item}" for i, item in enumerate(candidate_entities))
    candidate_prompt = f"The list of candidate answers is {candidate_entities}."

    task_query = "The goal question is: predict the tail entity [MASK] from the given {triplet} by completing the sentence '{query}'. Based on your own background knowledge and the knowledge I provide, select as many valid and correct answers as possible from the list of candidate answers. Please make sure to answer this question comprehensively, taking into account all the background knowledge you have. The output format of the correct candidate answer is strictly in accordance with <The correct candidate answers are [answer1, answer2, ...]>."

    prompt = f"{task_desc}\n {kg_info}\n {wiki_info}\n {candidate_prompt}\n {task_query}"

    if not args.use_relation_template:
        kg_info = f"**Knowledge from the knowledge graph**: {neighbors}."
        task_query = f"The goal question is: predict the tail entity [MASK] from the given {triplet}. Based on your own background knowledge and the knowledge I provide, select as many valid and correct answers as possible from the list of candidate answers. Please make sure to answer this question comprehensively, taking into account all the background knowledge you have. The output format of the correct candidate answer is strictly in accordance with <The correct candidate answers are [answer1, answer2, ...]>."
        prompt = f"{task_desc}\n {kg_info}\n {wiki_info}\n {candidate_prompt}\n {task_query}"
...
def process_answer_select(answer, candidates, entity):
    match = re.search(r'\[(.*?)\]', answer)
    extracted_text = answer
    if match:
        extracted_text = match.group(1)
        if entity in extracted_text and entity in candidates:
            candidates.remove(entity)  # 先删除匹配的元素
            candidates.insert(0, entity)  # 将元素插入到第一位
    return extracted_text, candidates
...
if __name__ == "__main__":
    ...
    for test_file in test_files:
        ...
        for item in test_data:
            ...
            forward_extracted_text, forward_sorted_candidates = process_answer_select(forward_answer, forward_candidate_entities, tail)
            ...
            backward_extracted_text, backward_sorted_candidates = process_answer_select(backward_answer, backward_candidate_entities, head)
            ...
            answer = save_data(item, forward_prompt, backward_prompt, forward_sorted_candidates, backward_sorted_candidates, forward_source_rank, backward_source_rank, forward_llm_rank, backward_llm_rank, forward_extracted_text, backward_extracted_text)
